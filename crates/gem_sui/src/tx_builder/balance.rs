use crate::{SUI_FRAMEWORK_PACKAGE, SuiError, tx_builder::move_call};
...
pub fn into_balance(txb: &mut TransactionBuilder, coin_type: &str, coin: Argument) -> Result<Argument, SuiError> {
    move_call(txb, SUI_FRAMEWORK_PACKAGE, MODULE_COIN, FUNCTION_INTO_BALANCE, &[coin_type], vec![coin])
}

pub fn from_balance(txb: &mut TransactionBuilder, coin_type: &str, balance: Argument) -> Result<Argument, SuiError> {
    move_call(txb, SUI_FRAMEWORK_PACKAGE, MODULE_COIN, FUNCTION_FROM_BALANCE, &[coin_type], vec![balance])
}

pub fn balance_zero(txb: &mut TransactionBuilder, coin_type: &str) -> Result<Argument, SuiError> {
    move_call(txb, SUI_FRAMEWORK_PACKAGE, MODULE_BALANCE, FUNCTION_BALANCE_ZERO, &[coin_type], vec![])
}

pub fn balance_value(txb: &mut TransactionBuilder, coin_type: &str, balance: Argument) -> Result<Argument, SuiError> {
    move_call(txb, SUI_FRAMEWORK_PACKAGE, MODULE_BALANCE, FUNCTION_BALANCE_VALUE, &[coin_type], vec![balance])
}

pub fn destroy_zero_balance(txb: &mut TransactionBuilder, coin_type: &str, balance: Argument) -> Result<(), SuiError> {
    move_call(txb, SUI_FRAMEWORK_PACKAGE, MODULE_BALANCE, FUNCTION_BALANCE_DESTROY_ZERO, &[coin_type], vec![balance])?;
    Ok(())
}
